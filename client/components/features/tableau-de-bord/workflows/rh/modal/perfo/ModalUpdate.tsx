...
import type { UpdatePerfoDataDTO } from '@/domains/dto/perfoData.dto';

interface ModalUpdatePerformanceProps {
  open: boolean;
...
export function ModalUpdatePerformance({ 
  open, 
  onOpenChange, 
  onSuccess, 
  perfoData,
  employees 
}: ModalUpdatePerformanceProps) {
  ...
  useEffect(() => {
    if (perfoData) {
      const empId = typeof perfoData.employeeId === 'string' 
        ? perfoData.employeeId 
        : perfoData.employeeId?._id || '';
      
      setPerfoForm({
        employeeId: empId,
        score: perfoData.score.toString(),
        tache: perfoData.tache || '',
        periode: perfoData.periode
      });
    }
  }, [perfoData]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const updatedData: UpdatePerfoDataDTO = {
      ...
      await PerfoDataRepoAPI.update(perfoData._id, updatedData);
      
      console.log("Performance modifiée avec succès");
      onSuccess();
      onOpenChange(false);
    } catch (error) {
      console.error("Erreur lors de la modification de la performance:", error);
      alert("Une erreur est survenue lors de la modification de la performance.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Modifier une performance</DialogTitle>
          <DialogDescription>
            Modifiez les données de performance
          </DialogDescription>
        </DialogHeader>
